            ...
            <div className="w-full max-w-xl flex items-center gap-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl px-5 py-4 shadow-xl">
              <input
                ...
                onChange={(e) => {
                  ...
                  setSearchQuery(val);
                  const results = recordsData.records.filter(
                    (record) =>
          ...
          {filteredRecords.length > 0 ? (
                ...
                >
                  <thead className="sticky top-0 bg-slate-900/90 backdrop-blur-xl">
                    <tr>
                      ...
                      <th className="px-6 py-5 text-left font-semibold">Email ID</th>
                    ...
                    {filteredRecords.map((record, index) => {
                      return (
                        <tr
                          ...
                          key={index}
                        >
                          <td className="px-6 py-4">{record.isbn}</td>

                          <td className="px-6 py-4">{record.studentEmail}</td>

                          <td className="px-6 py-4">
                            {new Date(record.borrowedDate).toDateString()}
                          </td>

                          <td className="px-6 py-4">{new Date(record.dueDate).toDateString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div className="flex justify-center mt-16">
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl px-12 py-8 shadow-xl">
                <p className="text-white text-2xl font-semibold text-center">No Records found :(</p>
              </div>
            </div>
          )}
        </div>
