...
export const getAdminBooks = async (req, res, next) => {
  try {
    ...
    await redisClient.setEx('lms:books', DEFAULT_EXPIRATION, JSON.stringify(books));
    return res.status(200).json({
      success: true,
      books,
    });
...
export const borrow = async (req, res, next) => {
  try {
    ...
    const x = await Student.findByIdAndUpdate(
      student._id,
      { $push: { booksBorrowed: book._id }, $inc: { booksBorrowedCount: 1 } },
      { new: true }
    );
...
export const returnBook = async (req, res, next) => {
  try {
    ...
    // const y = await Book.findByIdAndUpdate(bookId, { $inc: { count: 1 } }, { new: true });
    await redisClient.del('lms:books');
    await Student.findByIdAndUpdate(
      decoded.student,
      { $pull: { booksBorrowed: bookId }, $inc: { booksBorrowedCount: -1 } },
      { new: true }
    );
    const book = await Book.findById(bookId);
